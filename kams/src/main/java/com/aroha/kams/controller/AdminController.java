...
public class AdminController {
	...
	// Get Team Api Based On Department and Project
	...
	// Get All Team Api Based On Department and Project
	@GetMapping("getAllTeam")
	public ResponseEntity<?> getTeam() {
		List<TeamEntity> teamNameList = adminService.findAllTeam();
		return ResponseEntity.ok(teamNameList);
	}
	...
	public ResponseEntity<?> getallUser() {
		List<UserEntity> userList = adminService.findAll();
		return ResponseEntity.ok(userList);
	}

	@PostMapping("deleteUser")
	public ResponseEntity<?> deleteUser(@RequestBody UserPayload userPayload) {
		UserPayload getuserPayload = adminService.deleteUser(userPayload);
		return ResponseEntity.ok(getuserPayload);
	}

	@PostMapping("editUser")
	public ResponseEntity<?> editUser(@RequestBody UserPayload userPayload) {
		UserPayload getuserPayload = adminService.editUser(userPayload);
		return ResponseEntity.ok(getuserPayload);
	}

	@PostMapping("editCompany")
	public ResponseEntity<?> editCompany(@RequestBody CompanyPayload companyPayload) {
		CompanyPayload getcompanyPayload = adminService.editCompany(companyPayload);
		return ResponseEntity.ok(getcompanyPayload);
	}
	
/*
	@PostMapping("assigneUser")
	public ResponseEntity<?> assigneTeam(@RequestBody UserPayload userPayload) {
		System.out.println(userPayload);
		UserPayload getcompanyPayload = adminService.assigneTeam(userPayload);
		return ResponseEntity.ok(getcompanyPayload);
	}
...
*/
	//Assign Team Testing
	@PostMapping("assigneUser")
	public ResponseEntity<?> assignTeamDemo(@RequestBody UserAssignTeamPayload payload){
		List<TeamPayloadNew> list=payload.getUserTeamName();
        AssignResponsePayload getCompanyPayload = adminService.assignTeamDemo(payload,list);
		return ResponseEntity.ok(getCompanyPayload);
	}
